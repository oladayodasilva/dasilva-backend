...
const validateOrder = [
  body('user.fullName').notEmpty().withMessage('User full name is required'),
  body('user.email').isEmail().withMessage('Valid user email is required'),
  body('cartItems')
    .isArray({ min: 1 })
    .withMessage('Cart items must be a non-empty array'),
  body('cartItems.*.name').notEmpty().withMessage('Item name is required'),
...
router.post('/', validateOrder, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    console.warn('⚠️ Order validation failed:', errors.array());
    return res
      .status(422)
  ...
  try {
    ...
    // Verify payment (skip Paystack in development)
    let txn;
    if (isDev) {
      console.log('⚙️ Skipping Paystack verification (DEV mode)');
    ...
    if (txn.status !== 'success') {
      return res
        .status(400)
        .json({ message: 'Payment not verified', details: txn });
    }
